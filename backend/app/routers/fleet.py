...
from app.models import Vehicle, Driver, User, VehicleStatus, DriverStatus
...
def delete_vehicle(vehicle_id: int, db: Session = Depends(get_session)):
    ...
    # Soft delete: update status to keep historical logs functional for charts
...
def delete_driver(driver_id: int, db: Session = Depends(get_session)):
    ...
    # Soft delete: suspend status to protect operational continuity metrics
